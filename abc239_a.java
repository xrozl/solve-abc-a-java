...
public class Main {
    ...
    static class Calc{
        ...
        void abf(String o,Object z){System.out.printf(o,z);}
        ...
        int mod(int a,int b,int c){return mod(mod(a,b),c);}
        int avg(int... i){return sum(i)/i.length;}
        long avg(long... l){return sum(l)/l.length;}
        double avg(double... d){return sum(d)/d.length;}
        double sqrt(double d){return Math.sqrt(d);}
        Calc(){}
        void done() {
            long h = ni();
            ab(new BigDecimal(sqrt(h * (12800000 + h))).setScale(0, RoundingMode.HALF_UP).toString());
        }
    }
}